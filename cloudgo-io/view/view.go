...
import (
	...
	"github.com/tpisntgod/Service_Calculation/cloudgo-io/controller"
)

var htmlpath = "src/github.com/tpisntgod/Service_Calculation/cloudgo-io/templates/"
var pathtoAdd = "/src/github.com/tpisntgod/Service_Calculation/cloudgo-io/templates/mainpage.html"
...
func GetTodoListInformation(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		/*
			cookie, cookieError := r.Cookie("username")
			if cookieError != nil {
				todoListInformation = todoListInfo{FirstTerm: "please log in first", SecondTerm: "Ordinary User"}
			} else {
				todoListInformation = todoListInfo{FirstTerm: cookie.Value, SecondTerm: "Ordinary User"}
			}
		*/
		r.ParseForm()
